<template>
  <v-row>
    ...
    <v-dialog v-model="showRedirectDialog" width="400" persistent>
           <v-card 
             loading border
             class="bg-grey-darken-4 text-center">
            <v-card-title class="">
  ...
  if (form.value.validate()) {
    ...
    if (!checkIsMovieExist) {
     ...
     timeouts.push( setTimeout(async () => {
          ...
          // redirect to /moveiesView
            show.value = false
       showRedirectDialog.value = true
            
       timeouts.push(setTimeout(() => {
         router.push('/movies')
         store.btnName = 'Factory'
       }, 1000))
      ...
      timeouts.push(  setTimeout(() => (showErrorAlert.value = false), 1500))
      }
    } else {
...
const exitBtn = () => {
  show.value = false
  showRedirectDialog.value = true
 timeouts.push( setTimeout(() => router.push('/movies'),600))
}
