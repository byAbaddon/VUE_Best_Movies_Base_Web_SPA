 ...
 <div>
   <v-app-bar fixed height="60" color="deep-purple darken-2" image="/assets/images/header.png">
    ...
    <div v-if="!isEffect" class="d-flex justify-evenly align-center">
      ...
      <v-btn @click="onPath" class="text-shadow font-weight-bold">{{ store.btnName }}</v-btn>
...
const updateTitleVisibility = () => {
 ...
 timeouts.push(   setTimeout(() => showTitle.value = false, 2200))
  }
}
...
const hideToolbarButtons = () => {
  if (mediaQuery.matches) {
    ...
    isEffect.value = true
    timeouts.push( setTimeout(() => isEffect.value = false, 2200))
  }
}
...
const onPath = e => {
  ...
  let path = e.target.innerText.toLowerCase()
    // console.log(path);
  store.btnName = path == 'movies' ? 'Factory' : 'Movies'
