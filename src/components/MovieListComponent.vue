<template>
  <div>
    <v-card v-if="!store.currentMovies?.length" color="black" loading>
    ...
    <v-row class="rr my-1 pt-2">
      <v-col v-for="(movie, id) in store.currentMovies" :key="id" class="d-flex child-flex mb-3">
        <!--  ======= create cards ========== -->
        <v-card
          v-if="movie && movie.id"
          :id="movie.id"
        ...
        >
          ...
          <!--  divider  -->
          <v-divider class="mx-12 py-2" color="light-blue-lighten-5" thickness="2"></v-divider>
          ...
          <!--  divider  -->
          <v-divider class="mx-12 mt-3" color="light-blue-lighten-5" thickness="2"></v-divider>

          <p class="d-flex justify-center align-center text-grey-lighten-1">
            Year: {{ movie.year }}
          </p>
          <v-card-actions class="d-flex justify-between align-center" style="min-width: 16em">
            <v-btn class="bg-orange mx-auto" variant="flat" @click="showMovieDetailsBtn">
              Details</v-btn
            >

            <v-btn
              v-show="store.isAdminLogin"
              min-width="80px"
              class="bg-blue mx-3"
              variant="flat"
              @click="editMovieBtn"
...
const t1 = setTimeout(() => {
  message.value = 'Try connect to data base.'
}, 5000)
const t2 = setTimeout(() => {
  message.value = 'Please Wait or Reload.'
}, 10000)
const t3 = setTimeout(() => {
...
const showTitle = (title = '') => (title.includes('/') ? title.split('/')[1] || title : title)
...
.movie-card {
  ...
  box-shadow:
    -1px 0 10px rgba(139, 136, 136, 0.7),
    0 1px 10px rgba(243, 241, 241, 0.6);

  background:
    radial-gradient(circle at top, rgba(255, 140, 0, 0.08), transparent 40%),
    linear-gradient(135deg, #202035, #111010);
}
.movie-card:hover {
  padding: 1px;
  cursor: grab;
