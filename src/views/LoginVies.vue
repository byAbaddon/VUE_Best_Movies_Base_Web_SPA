...
timeouts.push(  setTimeout(() => (showProgressCircular.value = false), 2000))

  //only admin allowed login
  if (email.value != 'administrator@abv.bg') {
    errorMessage.value = "Wrong admin name or password!"
    showErrorAlert.value = true
  timeouts.push(   setTimeout(() => (showErrorAlert.value = false), 2000))
    return
  }
  ...
  if (localStorage.getItem("auth")) {
    ...
    timeouts.push(  setTimeout(() => ( router.push('/factory'), 500)))
